...
const members = computed(() => store.state.members.data);
const loading = computed(() => store.state.members.loading);

store.dispatch('getMembers', { search: search.value});
...
{
store.dispatch('deleteAllMember', checked)
.then(()=>{
  store.commit('notify', {
    type: 'success',
    message: 'Member was successfully deleted!'
  ...
  <table>
    ...
    <thead>
      <tr>
        ...
        <th />
        <th field="firstname" :sort-field="sortField" :sort-direction="sortDirection"
                           @click="sortMembers('firstname')">
                           First Name
          </th>

        <th>Last Name</th>
        <th field="day_left" :sort-field="sortField" :sort-direction="sortDirection"
        @click="sortMembers('day_left')">Day Left</th>
        <th field="status" :sort-field="sortField" :sort-direction="sortDirection"
        @click="sortMembers('status')">Status</th>
        <th field="price" :sort-field="sortField" :sort-direction="sortDirection"
        @click="sortMembers('price')">Total Price</th>
    ...
    <tbody v-else>
      ...
      <td data-label="Price" class="lg:w-1 whitespace-nowrap">
              <router-link
              :to="{ name: 'Payment', params: { id: member.id } }"
              >

          <b
            class="text-gray-500 dark:text-slate-400"

            >{{ $filters.currencyUSD(member.price) }}</b
          >
      </router-link>
