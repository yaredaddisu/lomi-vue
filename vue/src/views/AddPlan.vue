...
import { mdiBallotOutline,mdiCashMultiple,mdiCashClock, mdiAccount,mdiAsterisk, mdiMail, mdiGithub ,mdiAccountPlus,  mdiAccountMultiple
} from "@mdi/js";
...
const loading = computed(() => store.state.current_Price.loading);
...
let model = ref({
 ...
 watch(
  () => store.state.current_Price.data,
  (newVal, oldVal) => {
    model.value = {
 ...
 if (route.params.id) {
  store.dispatch("getPlan", route.params.id);
}
...
function savePrice() {
 let action = "created";
  ...
  store.dispatch("savePrice", { ...model.value }).then(({ data }) => {
    store.commit("notify", {
      type: "success",
      message: "Plan was successfully " + action,
    });
...
/>
  ...
  <CardBox class="shadow-xl" >
   ...
   <form v-else @submit.prevent="savePrice">
   ...
   <span
     >
      <h1  class="text-xl font-bold text-white">
       ...
       {{ route.params.id ? "Edit Plan" : "Add a Plan" }}
     </h1>
      </span
   >
  </NotificationBarInCard>
  <FormField label="Personal Information">
   <FormControl v-model="model.planType" :icon="mdiCashMultiple"  placeholder="Plan Type" />
   <FormControl v-model="model.price" :icon="mdiCashMultiple" placeholder="Price"/>
   <FormControl v-model="model.discount" :icon="mdiCashMultiple" placeholder="Discount"/>
...
<div class="w-full mb-4 border border-gray-200 rounded-lg bg-gray-50 dark:bg-gray-700 dark:border-gray-600">
   ...
   <div class="px-4 py-2 bg-white rounded-b-lg dark:bg-gray-800">
       ...
       <textarea id="editor" v-model="model.description" rows="8" class="block w-full px-0 text-sm text-gray-800 bg-white border-0 dark:bg-gray-800 focus:ring-0 dark:text-white dark:placeholder-gray-400" placeholder="Write an article..." required></textarea>
   </div>
</div>
    <!-- Status -->
